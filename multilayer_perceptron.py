...
from .base_hydro_model import BaseHydroModel
from .hydro_helpers._utils import _mse
...
class HydroMLP(BaseMultilayerPerceptron, BaseHydroModel, RegressorMixin):
    """ HydroMLP 
        
        Hydrological model using multi-layer perceptron
    """
    def __init__(self, hidden_layer_sizes=(4,), activation="relu",
                 solver='lbfgs', alpha=0.0001,
                 batch_size='auto', learning_rate="constant",
                 learning_rate_init=0.001,
                 power_t=0.5, max_iter=200, shuffle=True,
                 random_state=None, tol=1e-4, epsilon=1e-8, 
                 verbose=False, warm_start=False, momentum=0.9,
                 nesterovs_momentum=True, early_stopping=False,
                 validation_fraction=0.1, beta_1=0.9, beta_2=0.999, #--------#
                 shift_RE=5, shift_Q=2,
                 steps_predict=1, cv_decisioner='NSE', n_series_split=3,
                 slice_testing_set=False, training_with_cv=True,
                ):

        sup = super(HydroMLP, self)
        sup.__init__(hidden_layer_sizes=hidden_layer_sizes,
                     activation=activation, solver=solver, alpha=alpha,
                     batch_size=batch_size, learning_rate=learning_rate,
                     learning_rate_init=learning_rate_init, power_t=power_t,
        ...
        BaseHydroModel.cv_decisioner = cv_decisioner
        BaseHydroModel.n_series_split = n_series_split
        BaseHydroModel.slice_testing_set = False
        BaseHydroModel.training_with_cv = training_with_cv
    ...
    def deep_clone(self, zygote=None, overwrite=True):
        """ Deep Clone

            Clone a ML model, parameters, hyperparameters, as well as all other attributes.
        """
        zygote = zygote or self

        if zygote and isinstance(zygote, BaseEstimator):
            child = zygote.__class__()
            genes = zygote.__getstate__()
            for key, trait in genes.items():
                child.__setattr__(key, trait)
                if overwrite: self.__setattr__(key, trait)
        else:
            raise Exception("The object to clone must be an sklearn Estimator!")

        return child

    def save_pars_to_json(self, coefs=None, path=''):
        import pathlib, json
        if type(coefs) != 'list' and coefs != None:
            raise Exception("Coefficients are expected as a list of"
                " names but got %s of type '%s'"%(str(coefs), type(coefs)))
        
        if not coefs:
            coefs = ["weights", "bias"]

        for j, n in zip([self.coefs_, self.intercepts_], coefs):
            j_converted = []
            filename = path+n+'.json'

            for jj in j: j_converted.append(jj.tolist())

            js =  json.dumps(j_converted, indent=4)
            
            pathlib.Path(path).mkdir(parents=True, exist_ok=True) 

            with open(filename, "w") as f: f.write(js)

            print("%s saved to %s ...OK"%(n, filename))

    def _fit_lbfgs(self, X, y, X_val, y_val, activations, deltas, 
        ...
        print("X: %d X_val: %d y: %d y_val: %d" % (len(X),len(X_val), len(y), len(y_val)))
        optimal_parameters, self.loss_, d = fmin_l_bfgs_b(
            x0=packed_coef_inter,
            func=self._loss_grad_lbfgs,
            maxfun=self.max_iter,
            ...
            iprint=iprint,
            factr=self._render_tol_for_lbfgs(),
            pgtol=self.tol,
            maxls=100,
            args=(X, y, activations, deltas,
                X_val, y_val, coef_grads, intercept_grads))
        self._post_lbfgs(d)

        self._unpack(optimal_parameters)

    def _post_lbfgs(self, d):
        """ Post processing L-BFGS
        """
        self._d_lbfgs = d
        self.n_iter_ = d['funcalls']
        if d['warnflag'] == 0:
            print('L-BFGS converged with %d iterations.'%d['funcalls'])
        elif d['warnflag'] == 1:
            print('Warning : L-BFGS didn\'t reach convergence with %d'
                ' function calls and %d internal iterations.\n' % (
                    d['funcalls'], d['nit']))
            if self.diver_redo:
                self.tol = self.tol * 0.1
                print("Retraining the model with lower tolerance: \n"
                    "Current tolerance: %.1E" % self.tol)
                self._fit_lbfgs(X, y, activations, deltas, coef_grads,
                            intercept_grads, layer_units)
        else:
            print('Unexpected Fortran error with L-BFGS: ',d['task'])
            print('\n')

    def _render_tol_for_lbfgs(self, tol=None):
        """ Render pgtol for scipy.optimize.fmin_l_bfgs_b

            pgtol: 1e12 ==> low 
        """
        tol = tol or self.tol
        if tol >= 1e-2:
            return 1e10 * 1e-2 / tol
        elif tol >= 1e-4:
            return 1e5 * 1e-4 / tol
        else:
            return 10.0 * 1e-6 / tol

    def _loss_grad_lbfgs(self, packed_coef_inter, X, y,
                        activations, deltas, X_val, y_val,
                        coef_grads, intercept_grads):
        """Compute the MLP loss function and its corresponding derivatives
        with respect to the different parameters given in the initialization.
    ...
    def _fit_stochastic(self, X, y, X_val, y_val, activations, deltas,
        ...
        # Deleted early stopping here in _fit_stochastic(), put it in _fit() instead
        n_samples = X.shape[0]
